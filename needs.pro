...
function PowerSpectrum3dB, inputArray
  ...
  outputSpectrum = FLTARR(nWaveNumber)
  outputnumber = FLTARR(nWaveNumber)

  FOR k=0,nX-1 DO BEGIN
    FOR j=0,nX-1 DO BEGIN
      FOR i=0,nX-1 DO BEGIN

        kk =( SQRT((i - nWaveNumber)*(i - nWaveNumber) + (j - nWaveNumber)*(j - nWaveNumber) + (k - nWaveNumber)*(k - nWaveNumber)))
        if kk le nWaveNumber-1 then begin
        outputSpectrum(kk) = outputSpectrum(kk) + fftArray(i,j,k)
        outputNumber(kk) = outputNumber(kk) + 1
        endif
      ENDFOR
    ENDFOR
  ENDFOR

  RETURN, outputSpectrum / outputNumber
end
